...
#import "SinaWeibo.h"//微博
#import "SinaWeiboRequest.h"//请求
#import "SinaWeiboAuthorizeView.h"//认证页面
#import "SinaWeiboConstants.h"//常量
...
- (void)dealloc
{
    ...
    logInDidCancelBlock = nil;
    
    [logInDidFailBlock release];
    logInDidFailBlock = nil;
    
    [accessTokenInvalidOrExpiredBlock release];
    accessTokenInvalidOrExpiredBlock = nil;
...
- (void)requestDidFailWithInvalidToken:(NSError *)error
{
    ...
    if (accessTokenInvalidOrExpiredBlock)
    {
        accessTokenInvalidOrExpiredBlock(self,error);
    }
}
...
- (void)logInDidCancel
{
    ...
    if (logInDidCancelBlock)
    {
        logInDidCancelBlock(self);
    }
}
...
- (void)logInDidFailWithErrorInfo:(NSDictionary *)errorInfo
{
    ...
    else
    {
        ...
        if (logInDidFailBlock)
        {
            logInDidFailBlock(self,error);
        }
