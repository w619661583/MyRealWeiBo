...
-(id)init
{
    OHAttributedLabel *la=[[OHAttributedLabel alloc]init];
    la.delegate=self;
    if(self = [super init])
    {
        _isHasImg = NO;
        ...
        self.reUser_arr = [[NSMutableArray alloc] init];
        self.topic_arr = [[NSMutableArray alloc] init];
        self.reTopic_arr = [[NSMutableArray alloc] init];
    }
    return self;
}
...
-(void)setRetweeted_status:(NSDictionary *)retweeted_status
{
    ...
    self.reText =[_retweeted_status valueForKey:@"text"];
    //处理图文混排，并得到结果
    NSMutableDictionary* dic = [NSMutableDictionary dictionary];
    [self creatAttributeString:self.reText Font:self.reTextFont Dic:dic];//获得dic
    
    self.reTextHeight = [[dic objectForKey:TEXTHEIGHT] intValue];
    [self.attReString setAttributedString:[dic objectForKey:ATTSTR]];
    [self.imgs2 addObjectsFromArray:[dic objectForKey:IMGARR]];
    [self.reUser_arr addObjectsFromArray:[dic objectForKey:USERS]];
    [self.reTopic_arr addObjectsFromArray:[dic objectForKey:TOPICS]];
    
    [self.attReString setFont:self.reTextFont];
    self.rethumbnail_pic = [_retweeted_status valueForKey:@"thumbnail_pic"];
    self.rebmiddle_pic = [_retweeted_status valueForKey:@"bmiddle_pic"];
    self.reoriginal_pic = [_retweeted_status valueForKey:@"original_pic"];
    
    if(self.reText.length)
    {
        _isHasReText = YES;
    }
    if(self.rethumbnail_pic)
    {
        _isHasReImg = YES;
    }
}
...
-(StatuseType)type
{
    ...
    if(_isHasImg)
    {
        return StatuseTypeTextImage;
    }
    else if(_isHasReImg)
    {
        return StatuseTypeReImage;
    }
    else if(_isHasReText)
    {
        return StatuseTypeReText;
    }
    else
    {
        return StatuseTypeText;
    }
...
-(int)getTotalHeight//计算获得cell的总高度
{
    ...
    switch (self.type) {
        ...
        case StatuseTypeTextImage:
            total = self.screenNameHeight + self.textHeight + img_Height + create_date_Height + offset_y*4;
            break;
        case StatuseTypeReText:
            total = self.screenNameHeight + self.textHeight + self.reTextHeight +  create_date_Height + offset_y*6;
            break;
        case StatuseTypeReImage:
            total = self.screenNameHeight + self.textHeight + self.reTextHeight + img_Height + create_date_Height + offset_y*7;
            break;
    }
...
-(void)creatAttributeString:(NSString*)theText Font:(UIFont*)aFont Dic:(NSMutableDictionary*)theDic//处理图文混排
{
    ...
    //进行表情的html转化
    NSString *text = [self transformString:theText];
    ...
    NSMutableAttributedString* temp = [p attrStringFromMarkup:text];
...
-(NSArray *)regexUsers:(NSString *)o_text
{
    ...
    NSArray *array_user = [text componentsMatchedByRegex:regex_user];
    return array_user;
}
